...
export async function getServicesPaginated(
  ...
  const [dataResult, countResult] = await Promise.all([
    status
      ? query<Service>(
          `SELECT * FROM "PitStop_direction".service WHERE status = $1 ORDER BY start_date DESC LIMIT $2 OFFSET $3`,
          [status, pageSize, offset]
        )
      : query<Service>(
          `SELECT * FROM "PitStop_direction".service ORDER BY start_date DESC LIMIT $1 OFFSET $2`,
          [pageSize, offset]
        ),
    status
      ? query<{ count: string }>(
          'SELECT COUNT(*)::text as count FROM "PitStop_direction".service WHERE status = $1',
          [status]
        )
      : query<{ count: string }>(
          'SELECT COUNT(*)::text as count FROM "PitStop_direction".service',
          []
        )
  ]);
